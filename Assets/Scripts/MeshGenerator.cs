...
public class MeshGenerator : MonoBehaviour
{
    ...
    public int textureTileAmount = 1;
    ...
    public void GenerateMesh(int[,] map, float squareSize) {
        ...
        for(int i = 0; i < vertices.Count; i++) {
            float percentX = Mathf.InverseLerp(-map.GetLength(0)/2*squareSize, map.GetLength(0)/2*squareSize, vertices[i].x) * textureTileAmount;
            float percentY = Mathf.InverseLerp(-map.GetLength(0)/2*squareSize, map.GetLength(0)/2*squareSize, vertices[i].z) * textureTileAmount;
